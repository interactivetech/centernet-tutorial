...
#   gt = gt.unsqueeze(1).float()
  ...
  pos_loss = torch.logsig (pred) * torch.pow(1 - pred, 2) * pos_inds
...
#   gt = gt.unsqueeze(1).float()
  ...
  if num_pos == 0:
    ...
    loss =  loss - (pos_loss + neg_loss) / (num_pos+1e-4)
  return loss
def centerloss4(pred_hm, hm,pred_regr, regr,mask,inds,wh,weight=0.4, size_average=True):
    # Binary mask loss
    '''
    pred_hm: [4,1,128, 128]
    hm: [4,1,128, 128]
    ...
    mask_loss = neg_loss3(pred_hm,hm)
    # mask = hm>0.99  
    # Regression L1 loss
